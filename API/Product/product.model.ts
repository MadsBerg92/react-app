import fs from "../../node_modules/fs.promises"
...
export async function saveProducts(product: Product[] = [], data: Data): Promise<void>{
...
export async function getByID(prodId: number): Promise<Product[]>{
    ...
    const index = prodArr.findIndex((currentProd) => currentProd.productID === prodId);
    if(index === -1){
...
export async function getProdByCategory(category:string): Promise<Product[]> {
    ...
    const categoryArr = productArr.filter(product => product.category === category);
    return categoryArr
    
}

//Get product by category section
export async function getProdByCategorySection(section: string): Promise<Product[]>{
    const data = await readData();
    const prodArr:Product[] = []
    const categoryArr:string[] = [];
    data.category.forEach((c:{name:string, section:string, description:string}) => {
      if(c.section === section) {categoryArr.push(c.name)};
    });
...
if(prodArr.length === 0) throw new Error(`No products with the section: ${section}`);
return prodArr;
}
...
export async function getProductsList(products:{productID:number}[]): Promise<Product[]> {
    const data = await readData();
...
function findProduct(productID:number, data): Product{
    ...
    if(product === undefined) throw new Error(`Product with productID: ${productID} doesn't exist`)
    return product;
}